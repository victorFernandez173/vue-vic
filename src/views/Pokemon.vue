...
function obtenerNumPokemons() {
  servPoke.getNumberOfPokes(num.value).then(response => {
...
function pokemonPorNombre() {
  ...
  servPoke.getName(name.value.toLowerCase()).then(response => {
...
function obtenerFoto() {
  servPoke.getName(name2.value.toLowerCase()).then(response => {
    queryImg.value = response.data.sprites.other['official-artwork'].front_default;
    queryImg2.value = response.data.sprites.other['dream_world'].front_default;
...
onMounted(() => { getAllPoke() });
</script>

<template>
  <h2 class="text-center">POKEMON API</h2>
  <h5 class="text-center">Implementa "simplebar" una extension que usa una scrollbar más vistosa con el overflow</h5>
  ...
  <SimpleBar class="container-fluid text-center" id="idDos">
    ...
    <div class="row d-flex justify-content-center">
      <form class="col-10 col-sm-8 col-lg-6" @submit.prevent="obtenerNumPokemons">
        <div class="input-group mb-3 mt-3">
          <input v-model="num" type="number" class="form-control" placeholder="Número de pokemones a buscar: "
            aria-label="Recipient's username">
          <button class="btn btn-outline-secondary">Obtener {{ num
          }}</button>
        </div>
      </form>
    </div>
    <div v-if="num" class="col-lg-10 list-group text-center">
      ...
      <ul>
        <li v-for="poke, id in queryPokeNum"> {{ id + 1 }}.{{ poke.name.charAt(0).toUpperCase() + poke.name.slice(1)
        }}</li>
      </ul>
    </div>
  ...
  <SimpleBar class="container-fluid text-center" id="idTres">
    ...
    <div class="row d-flex justify-content-center">
      <form class="col-10 col-sm-8 col-lg-6" @submit.prevent="pokemonPorNombre">
        <div class="input-group mb-3 mt-3">
          <input v-model="name" type="text" class="form-control" placeholder="Nombre de pokemon a buscar: "
            aria-label="Recipient's username">
          <button class="btn btn-outline-secondary">Buscar {{ name
          }}</button>
        </div>
      </form>
    </div>
    <div v-if="queryName">
      ...
      <p><vue-json-pretty :deep=3 show-line-number show-icon :data="{ queryName }" /></p>
    </div>
    <p v-else>Sin coincidencias</p>
  </SimpleBar>
    ...
    <div>
      <div class="row d-flex justify-content-center">
        <form class="col-10 col-sm-8 col-lg-6" @submit.prevent="obtenerFoto">
          <div class="input-group mb-3 mt-3">
            <input v-model="name2" type="text" class="form-control" placeholder="Pokemon a buscar: "
              aria-label="Recipient's username">
            <button class="btn btn-outline-secondary">Buscar {{ name2
            }}</button>
          </div>
        </form>
      </div>
  ...
  <SimpleBar class="container-fluid" id="idUno">
    ...
    <div class="row d-flex justify-content-center">
      <div class="col-lg-10 list-group text-center">
        <a :href="poke.url" target="_blank" class="list-group-item list-group-item-info list-group-item-action"
          v-for="poke, id in veintePrimerosPokemones"> {{ id + 1 }}.{{
